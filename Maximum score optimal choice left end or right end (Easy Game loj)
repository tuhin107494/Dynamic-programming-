...
///====================================StartHere=================////////////
ll a[200],dp[105][105][2];

ll dyn(ll i,ll j,ll id)
{
    if(i>j)return 0LL;
    if(dp[i][j][id]!=-1)return dp[i][j][id];
      ...
      if(id==0)
      {
          ...
          for(ll k=j;k>=i;k--)
          {
              ...
              sum+=a[k];
              res=max(res,sum+dyn(i,k-1,1));
              
          }
          return dp[i][j][id]=res;
      }
      else
      {
